...
void mem_init(void)
{
	...
	// Find out how much memory the machine has (npages & npages_basemem). ////////////////// look for  IMPLEMENTATION !
	i386_detect_memory();
	...
	// struct PageInfo memory_array[npages];
...
page_init(void)
{
	...
	struct PageInfo *pages;		// Physical page state array
	static struct PageInfo *page_free_list;	// Free list of physical pages */
	// here we are making list of free & available pages
	...
	for ( i  =1 ; i < npages; i++) {
		...
		//if(!( page_free_list >= IOPHYSMEM && page_free_list =< EXTPHYSMEM)){
		if(i==0)
		{
			pages[i].pp_ref = 1;
...
//
pte_t *
pgdir_walk(pde_t *pgdir, const void *va, int create) // reviewwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww 
{
	// Fill this function in
	...
	struct PageInfo *newly_page=NULL ; // new page we'll allocate
	pde_t *pointer_table_page_index=NULL; // 
	pde_t* page_table=NULL ;
...
{
	// Fill this function in
	// pgdir->physaddr_t ts_cr3=*page_table_entry_index; for base register pointer

	pde_t *page_table_entry=NULL;

	int i;
	uint32_t n_size = size / PGSIZE ;
	for(i=0; i< n_size ; i++)
	{
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	pte_t *pyhsical_page_entry = NULL ;
...
{
	// Fill this function in
	struct PageInfo *page_at_va = NULL ;
	...
	page_at_va=pa2page( PTE_ADDR(*entry_of_page_table) ); // zero last 12 f MSB t LSB & retrun pg number of LA
